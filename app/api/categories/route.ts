...
export async function GET() {
  try {
    ...
    if (result.error) {
      ...
      result = {
        ...legacyResult,
        data: (legacyResult.data ?? []).map((category: Record<string, unknown>) => ({
          ...category,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const categoryResult = await supabase.from("categories").select("name").eq("id", id).maybeSingle();
    if (categoryResult.error) throw categoryResult.error;
    if (!categoryResult.data) {
      return NextResponse.json({ message: "카테고리를 찾을 수 없습니다." }, { status: 404 });
    }

    let relationResult = await supabase
      .from("artists")
      .select("id", { count: "exact", head: true })
      .eq("main_category_id", id);
    if (relationResult.error) {
      relationResult = await (supabase as any)
        .from("artists")
        .select("id", { count: "exact", head: true })
        .eq("genre", categoryResult.data.name);
    }
    if (relationResult.error) throw relationResult.error;
    if ((relationResult.count ?? 0) > 0) {
      return NextResponse.json(
